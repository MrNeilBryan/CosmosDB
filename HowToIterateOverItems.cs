...
public class Program // Added a Program class to contain Main and helper methods
{
    ...
    private static readonly string DatabaseId = "xxxxxx";
    ...
    public static async Task Main(string[] args)
    {
        ...
        Console.WriteLine("Iterating over all items in the container...");
        
        // Define the query to select all items. Note the alias can be anything, c is the usual
        string queryString = "SELECT * FROM whatever";
        QueryDefinition query = new QueryDefinition(queryString);

        using FeedIterator<Product> resultSetIterator = container.GetItemQueryIterator<Product>(query);

        while (resultSetIterator.HasMoreResults)
        {
            FeedResponse<Product> response = await resultSetIterator.ReadNextAsync();

            foreach (Product p in response)
            {
                Console.WriteLine($"ID: {p.id}, Name: {p.Name}, Price: £{p.Price}, Tags: [{string.Join(", ", p.Tags ?? Array.Empty<string>())}]");
            }
        }
